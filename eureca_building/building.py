...
""")
    ...
    def simulate(self,
        ...
        total["Grid Electricity Primary Non-Renewable Energy [kWh]",f"Bd {self.name}"]=1.95 * total["Taken from the Gird [kWh]",f"Bd {self.name}"]
        ...
        total["Grid Electricity Primary Renewable Energy [kWh]"]=0.47 * total["Taken from the Gird [kWh]",f"Bd {self.name}"]
        total["Gas CO2 Emission Scope 1 [kg CO2]",f"Bd {self.name}"]=1.05 * 1.055* 11.200 * 58.9 / 27777 * results ["Heating system gas consumption [Sm3]"][:, 0] /1000*60
        total["Oil CO2 Emission Scope 1 [kg CO2]",f"Bd {self.name}"]=1.07 * 9.333 * 741 / 27777 * results ["Heating system oil consumption [L]"][:, 0]/1000*60
        total["Wood CO2 Emission Scope 1 [kg CO2]",f"Bd {self.name}"]=1.00 * 4.860 * 889 / 27777 * results ["Heating system wood consumption [kg]"][:, 0]/1000*60
        total["Coal CO2 Emission Scope 1 [kg CO2]",f"Bd {self.name}"]=1.10 * 8.140 * 932 / 27777 * results ["Heating system coal consumption [kg]"][:, 0]/1000*60
        ...
        total["Gas CO2 Emission Scope 3 [kg CO2]",f"Bd {self.name}"]=1.05 * 1.055* 11.200 * 12.3 / 27777 * results ["Heating system gas consumption [Sm3]"][:, 0] /1000*60
        total["Oil CO2 Emission Scope 3 [kg CO2]",f"Bd {self.name}"]=1.07 * 9.333 * 12 / 27777 * results ["Heating system oil consumption [L]"][:, 0]/1000*60
        total["Wood CO2 Emission Scope 3 [kg CO2]",f"Bd {self.name}"]=1.00 * 4.860 * 32 / 27777 * results ["Heating system wood consumption [kg]"][:, 0]/1000*60
        total["Coal CO2 Emission Scope 3 [kg CO2]",f"Bd {self.name}"]=1.10 * 8.140 * 93.2 / 27777 * results ["Heating system coal consumption [kg]"][:, 0]/1000*60
